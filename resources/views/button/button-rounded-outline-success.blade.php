...
@if($href)
    <a href="{{ $href }}" {{ $attributes->merge(['class' => 'inline-flex items-center justify-center font-semibold text-green-700 shadow-xs ring-1 ring-green-300 ring-inset hover:bg-green-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-green-600 transition-colors duration-200 bg-white ' . $sizeClass]) }}>
        {{ $slot }}
    </a>
@else
